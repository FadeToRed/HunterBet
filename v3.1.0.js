/* =========================================================
   TORRE CELESTE BETTING — torre-celeste.js  (v3.1.0)
...
(function () {
  ...
  function start(db) {
    ...
    function applica_passaparola_n(quote, stats, ids) {
      ...
      if (n === 0) return q;
      if (n === 1) {
        q[0] = Math.max(1.2, Math.min(10, Math.round(q[0] * 10) / 10));
        return q;
      }
      ...
      if (tied.length === n && n_al_top === n) {
        for (var z = 0; z < n; z++)
          q[z] = Math.max(1.2, Math.min(10, Math.round(q[z] * 10) / 10));
        return q;
      }
      ...
      if (tied.length === n) {
        /* quote tutte pari ma stats diverse: -1 a chi ha più stats */
        q[winner] -= 1;
      } else {
      ...
      for (var j = 0; j < n; j++)
        q[j] = Math.max(1.2, Math.min(10, Math.round(q[j] * 10) / 10));
      return q;
    }
          ...
          .then(function(html_text) {
            ...
            if (mancanti > 0) {
              ...
              for (var a = 0; a < pool.length; a++) {
                ...
                var qv = BASE + pool[a].fama + aura;
                quote.push(Math.max(1.2, Math.min(10, qv)));
