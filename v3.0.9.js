      ...
      document.getElementById("tcb-m-ok").addEventListener("click", function() {
        ...
        fetch(lott.url_scheda)
          ...
          .then(function(html_text) {
            ...
            if (!altri_dati) {
              /* salva i dati in attesa del secondo contendente, senza calcolare nulla */
              row._quota_dati = {
              ...
              status.textContent = "Dati salvati. Calcola la quota anche dell'altro contendente per completare.";
              btn.disabled = false;
              btn.innerHTML = '<i class="fas fa-check"></i> Salvato';
              setTimeout(modal_close, 1400);
            } else {
              /* entrambi disponibili — calcola tutto con Passaparola */
              ...
              status.textContent = "Quote calcolate!";
              btn.disabled = false;
