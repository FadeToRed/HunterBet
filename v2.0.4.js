  ...
  function start(db) {
    ...
    function render_vecchi_scontri() {
      ...
      for (var i=0; i<list.length; i++) {
        ...
        for (var k=0; k<s.partecipanti.length; k++) {
          ...
          var ha_vinto = s.vincitore && (topic_id(p.url_scheda) === topic_id(s.vincitore_scheda) || p.nome === s.vincitore);
    ...
    function do_scommessa(scontro, sel_idx) {
      ...
      db.ref("scommesse/utenti/" + state.utente.uid + "/saldo").transaction(function(s) {
        if ((s||0) < importo) return; return (s||0) - importo;
      }).then(function(res) {
        if (!res.committed) {
          if(document.getElementById("tcb-m-err")) document.getElementById("tcb-m-err").textContent = "Saldo insufficiente.";
          if(btn){ btn.disabled=false; btn.innerHTML='<i class="fas fa-check"></i> Conferma'; } return;
        }
    ...
    function do_adm_vinci(sc_id, pidx) {
      ...
      }).then(function(){ toast("Vincite distribuite.", "ok"); })
